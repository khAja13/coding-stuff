...
public class DP_printLCS {
    ...
    public static void main(String[] args) {
        ...
        int i = str1.length(), j = str2.length(), index = dp[i][j];
        StringBuilder ans = new StringBuilder("");
        ans.setLength(index);
        while(i>0 && j>0) {
            if(str1.charAt(i-1) == str2.charAt(j-1)) {
                ans.setCharAt(index-1, str1.charAt(i-1));
                index--;
                i--;j--;
            }
        ...
        System.out.println("Longest common subsequence string is " + ans);
